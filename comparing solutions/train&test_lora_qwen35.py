...
OUTPUT_DIR = "/mnt/data/tangleyi/AI_Text_Detection/Deep_Sleep_Homework/outputs_pure_torch1"
CHECKPOINT_DIR = os.path.join(OUTPUT_DIR, "checkpoints")  # 存放每个 epoch 的 checkpoint
...
def evaluate(model, val_loader):
    ...
    with torch.no_grad():
        for batch in val_loader:
            ...
            logits = outputs.logits.float()          # 👈 转 float32
...
with open(log_file, "a" if start_epoch>0 else "w") as f:
    if start_epoch==0:
...
for epoch in range(start_epoch, EPOCHS):
    ...
    for step, batch in pbar:
        ...
        if (step + 1) % SAVE_STEPS == 0:
            ...
            }, checkpoint_path)  # checkpoint_path 始终是同一个文件，自动覆盖
